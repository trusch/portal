...
- Audio changes must keep the mobile unlock dance (suspended AudioContext + gesture-end resume kicks) and the iPad audio-session promotion (audioSession.type='playback' + silent <audio> keepalive, Apple-gated and >=1s — un-gated it crackles on Android) intact.
